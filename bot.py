...
class Browser:
    def __init__(self, driver_path, site):
        ...
        self.show_type            = "//div[text()='PCX DOLBY ATMOS']"
        ...
        self.browser = uc.Chrome(service=self.service, options=self.option)
    ...
    def is_prasads_available(self):
        try:
            prasads = self.browser.find_element("xpath", self.prasads_large_screen)
            # print(prasads)
            return True
        except Exception as e:
    ...
    def open_and_find_seats_available(self):
        ...
        try:
            show_time_list = self.browser.find_elements("xpath", self.show_type)
            for show_time in show_time_list:
                self.browser.execute_script("arguments[0].scrollIntoView(false);", show_time)
                show_time.click()
                try:
                    self.browser.find_element("xpath", self.accept_button).click()
                ...
                # sleep(5)
                try:
                    self.browser.find_element("xpath", self.close_btn).click()
                except Exception as e:
    ...
    def find_best_seats(self):
        ...
        try:
            layout = self.browser.find_element("xpath", self.seat_layout)
            rows = [tr for tr in layout.find_elements("xpath", ".//tr")]
            # Skipping dummy row at the top
            rows = rows[1:]
            seat_map = {}
            send_mail = False
            for row in rows:
                tds = [td for td in row.find_elements("xpath", ".//td")]
                row_id = tds[0].find_element("xpath", ".//div").text
                if((row_id != self.first_row) and (row_id != self.second_row) and (row_id != self.third_row)):
                    continue
                seats = [div for div in tds[1].find_elements("xpath", ".//div")]
                # Skipping whitespace coloumn
                seats = seats[1:]
                seats_available = []
                for seat in seats:
                    seat_number = seat.find_element("xpath", ".//a").text
                    status = seat.find_element("xpath", ".//a").get_attribute("class")
    ...
    def prepare_message(self, seat_map):
        ...
        try:
            message += self.browser.find_element("xpath", self.show_time).text
        except Exception as e:
